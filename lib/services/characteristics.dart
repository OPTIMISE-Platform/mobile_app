...
class CharacteristicsService {
  ...
  static Future<List<Characteristic>> getCharacteristics() async {
    ...
    bool cont = true;
    while (cont) {
      String uri = '${dotenv.env["API_URL"] ?? 'localhost'}/permissions/query/v3/resources/characteristics';
      ...
      final l = resp.data ?? [];
      cont = l.length == 9999;
      result.addAll(List<Characteristic>.generate(l.length, (index) => Characteristic.fromJson(l[index]["raw"])));
    }
    return result;
  }
}
